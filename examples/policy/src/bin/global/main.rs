...
fn main() -> anyhow::Result<()> {
    ...
    // Product assertions
    assert_matches!(
        policy.authorise(&product, &user, &ProductAction::Read),
        Ok(_)
    );

    assert_matches!(
        policy.authorise(&product, &user, &ProductAction::Write),
        Err(_)
    );

    assert_matches!(
        policy.authorise(&product, &user, &ProductAction::Delete),
        Err(_)
    );

    assert_matches!(
        policy.authorise(&product, &admin_user, &ProductAction::Read),
        Ok(_)
    );

    assert_matches!(
        policy.authorise(&product, &admin_user, &ProductAction::Write),
        Ok(_)
    );

    assert_matches!(
        policy.authorise(&product, &admin_user, &ProductAction::Delete),
        Ok(_)
    );
